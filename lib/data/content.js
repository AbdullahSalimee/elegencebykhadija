...
// Create (or replace wholesale). Used by POST, where the caller supplies a
// complete row.
...
// Partial edit. Deliberately an UPDATE rather than an upsert: an upsert builds
// a whole tuple, so patching one field would fail the NOT NULL check on every
// column the caller didn't send (announcements.message, hero_slides.title) and
// silently reset the ones that have defaults — sort_order back to 999. The
// "Live / Hidden" toggle sends nothing but `active`, so this matters.
export async function updateContent(table, id, fields) {
  if (!ADMIN_SELECTS[table]) throw new Error(`unknown content table: ${table}`);
  const db = supabaseServer();
